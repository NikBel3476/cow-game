...
export class LevelLoader {
  ...
  initLockDoors(
    lockDoors: ILevel["MapObjects"]["Interactive"]["LockDoor"]
  ): LockDoor[] {
    const lockDoorsArr: LockDoor[] = [];
    if (lockDoors) {
      (Object.keys(lockDoors) as DoorOrientation[]).forEach(
        (lockDoorOrientation) => {
  ...
  initAutoDoors(
    doors: ILevel["MapObjects"]["Interactive"]["AutoDoor"]
  ): AutoDoor[] {
  ...
  initPistons(
    pistons: ILevel["MapObjects"]["Interactive"]["Piston"]
  ): Piston[] {
  ...
  initButtons(
    buttons: ILevel["MapObjects"]["Interactive"]["Button"]
  ): Button[] {
    return (
      buttons?.map((button) => {
        return new Button(
