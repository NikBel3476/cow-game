...
export class Game {
  ...
  findCowByCoordinates(coordinates: Coordinates): Cow | undefined {
    return this._cows.find(
      (cow) =>
        cow.coordinates.x === coordinates.x &&
        cow.coordinates.y === coordinates.y
    );
  }

  findStaticFieldByCoordinates(
    coordinates: Coordinates
  ): IField | Arrow | undefined {
  ...
  findArrowByHtmlElement(htmlElement: HTMLElement): Arrow | undefined {
    return this._arrows.find(
      (arrow: Arrow) => arrow.linkedHtmlElement === htmlElement
    );
  }
  ...
  mainLoopFunc() {
    Object.values(this._cows).forEach((cow: Cow) => {
      const currentField: IField | Arrow | undefined =
        this.findFieldByCoordinates(cow.coordinates);
        ...
        if (
          ...
          if (currentField.color === "Red") {
            ...
            this._staticObjects.splice(
              this._staticObjects.indexOf(currentField),
              1
            );
      ...
      if (nextField?.impassable) {
        ...
        if (nextField instanceof LockDoor) {
          ...
          if (keys && keys.length !== 0) {
            ...
            this._staticObjects.splice(
              this._staticObjects.indexOf(nextField),
              1
            );
            this._interactiveFields.splice(
              this._interactiveFields.indexOf(nextField),
              1
            );
            cow.move();
          }
      ...
      if (nextField instanceof HayBale && cow.layer === 1) {
        switch (cow.direction) {
          case "Up": {
            ...
            if (
              ...
              nextField.coordinates.y =
                Math.round((nextCoordinates.y - 1) * 100) / 100;
              cow.move();
            }
            break;
          }
          case "Right": {
            ...
            if (
              ...
              nextField.coordinates.x =
                Math.round((nextCoordinates.x + 1) * 100) / 100;
              cow.move();
            }
            break;
          }
        ...
        if (fieldUnderHayBale instanceof Pit && fieldUnderHayBale.activated) {
          this._movableObjects.splice(
            this._movableObjects.indexOf(nextField),
            1
          );
          this._interactiveFields.splice(
            this._interactiveFields.indexOf(nextField),
