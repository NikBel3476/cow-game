class Render {
    ...
    drawStaticObjects(fields: Field[], mapArrows: Arrow[], goblet: Goblet) {
        Object.values(fields).forEach((field: Field) => {
            (this.gameTable[field.coordinates.y - 1][field.coordinates.x - 1].firstChild as HTMLElement)
                .style.background = `url("../../${field.imgUrl}") no-repeat center`;
        });
        ...
        (this.gameTable[goblet.coordinates.y - 1][goblet.coordinates.x - 1].firstChild as HTMLElement)
            .style.background = `url("../../${goblet.imgUrl}") no-repeat center`;
    }
