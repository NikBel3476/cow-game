...
export class Render {
  ...
  clearScene() {
    ...
    this.arrowsTable.forEach((row) =>
      row.forEach(
        (cell) => ((cell.firstChild as HTMLElement).style.background = "")
      )
    );
  }
  ...
  clearArrowsTable() {
    this.arrowsTable.forEach((row) =>
      row.forEach(
        (field) => ((field.firstChild as HTMLElement).style.background = "")
      )
    );
  }
  ...
  drawScene(
    staticElements: (IField | Arrow)[],
    movableElements: (Cow | HayBale)[]
  ) {
