...
export class UI {
  ...
  constructor(
    ...
    document
      ...
      ?.appendChild(this.htmlGameTable);
    this.htmlArrowsTable = this.createArrowsTable();
    document
      .querySelector(arrowsTableCssSelector)
      ?.appendChild(this.htmlArrowsTable);
    ...
    if (document.getElementById("modal-button") !== null)
      (
        document.getElementById("modal-button") as HTMLButtonElement
  ...
  getArrowTableElement(htmlElement: HTMLElement): boolean {
    return this.arrowsTable.some((row) =>
      row.some(
        (arrowElement) =>
          htmlElement === (arrowElement.firstChild as HTMLElement)
      )
    );
  ...
  showEndLevelModalWindow(): void {
    ...
    if (modalLabel !== null) {
      ...
      (document.getElementById("end-level-modal-img") as HTMLImageElement).src =
        img;
  ...
  showCompletedLevelsModalWindow() {
    const levelsListWrapper = document.getElementById('completed-levels-list-wrapper') as HTMLElement;
